...
Applying differential privacy to full vision transformer gradients in
federated settings imposes prohibitive communication cost and memory
pressure: transmitting $343$\,MB of full-model gradients per round and
constraining GPU batch size to $8$ at $85.8$M trainable parameters (limited
by per-sample gradient memory under Opacus).
...
count.
GPU experiments on PathMNIST ($5$ clients, $10$ rounds, $\noise{=}1.1$,
$\delta{=}10^{-5}$) confirm that \method{} achieves AUC $0.9464$
($\eps{=}7.46$, well within the clinical threshold $\eps \leq 8$) while
reducing per-round communication from $343$\,MB to $2.4$\,MB ($146{\times}$)
and training $1.8{\times}$ faster per round than full-model DP-FedAvg.
\end{abstract}
...
\emph{Memory pressure} is substantial: per-sample gradient computation under
Opacus at $85.8$M parameters constrains GPU batch size to $8$ on $8$\,GB
hardware, reducing effective data coverage per round and inflating the
RDP privacy budget.
In CPU prototype experiments at $\noise{=}1.1$ with minimal batch counts,
we observe AUC collapse from $0.997$ to $0.534$ on PathMNIST, consistent
with the $74$-study scoping review by
...
$85.8$\,M parameters.
LoRA adapters applied to \texttt{query} and \texttt{value} projections in
all $12$ attention layers with $r{=}16$, $\alpha_{\text{LoRA}}{=}32$,
dropout $0.05$ (HuggingFace ViT naming convention).
...
\subsection{Experiment 2: CPU Prototype --- Communication and Efficiency}
\label{sec:exp:dp}

\emph{Claim: \method{} reduces per-round communication by $145{\times}$ and
per-round training time by $1.8{\times}$ versus full-model DP-FedAvg;
these efficiency gains are hardware-independent and confirmed on CPU.}

Table~\ref{tab:cpu} reports the CPU prototype results (fast\_dev\_run=2,
$3$ clients, $8$ rounds).
As explained in the table note, both DP conditions show AUC $\approx 0.48$
in this degenerate setting because $2$ mini-batches per epoch are
insufficient for DP noise to be overcome by gradient signal.
Utility comparisons are therefore deferred to the GPU runs
(Experiment~3); only the efficiency columns are taken as evidence here.
...
\textbf{Uncontrolled batch-size comparison.}
The two GPU DP conditions (Experiment~3) differ in batch size ($8$ for
full-model DP-FedAvg, $32$ for \method{}) due to GPU memory constraints,
producing different sampling ratios and therefore different $\eps$ values
($5.17$ vs.\ $7.46$).
A controlled utility comparison at identical $(\noise, \clip, q)$ requires
either larger GPU memory or gradient checkpointing and is left for future
work.
