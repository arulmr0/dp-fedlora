...
\begin{verbatim}
# Full-scale baselines (see task_plan.md for full command list)
uv run python pipeline/train.py experiment.name=fedavg_dp_alpha05
uv run python pipeline/train.py model=vit_b16_lora \
  privacy=dp_sgd experiment.name=fedlora_dp_alpha05

# Non-IID sweep (alpha=0.1, 1.0)
uv run python pipeline/train.py model=vit_b16_lora \
  privacy=dp_sgd dataset.alpha=0.1
uv run python pipeline/train.py model=vit_b16_lora \
  privacy=dp_sgd dataset.alpha=1.0
\end{verbatim}
